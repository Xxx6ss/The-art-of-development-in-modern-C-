...
struct AddressHasher {
    ...
    unsigned long operator() (const Address& adress) const {
        return num * num * s_hash(adress.city) + num * s_hash(adress.street)
                + i_hash(adress.building);
    }
    int num = 2'946'901;
    hash<string> s_hash;
    hash<int> i_hash;
...
struct PersonHasher {
    ...
    unsigned long operator() (const Person& person) const {
        return num * num * num * num * s_hash(person.name) + num * num * num *
        i_hash(person.height) + num * num * d_hash(person.weight) + addr_hash(person.address);
    }

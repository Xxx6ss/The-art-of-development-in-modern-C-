//
//  main.cpp
//  Const in concurrent_map
//
//  Created by Andrew Kireev on 21.08.2020.
  ...
  using MapType = unordered_map<K, V, Hash>;
    ...
    struct WriteAccess {
       ...
       V& ref_to_value;
        ...
        WriteAccess(const K& key, Bucket& data)
            : g(data.m), ref_to_value(data.data[key])
        {
        }
     };
     ...
     struct ReadAccess {
       ...
       const V& ref_to_value;
         ...
         ReadAccess(const K& key, const Bucket& data)
         : g(data.m), ref_to_value(data.data.at(key))
         {
         }
     };
    ...
    WriteAccess operator[](const K& key) {
        size_t index = hasher(key) % maps.size();
   ...
   MapType BuildOrdinaryMap() const {
        ...
        for (const auto& item : maps) {
            lock_guard<mutex> g(item.m);
            new_map.insert(item.data.begin(), item.data.end());
        }
        return new_map;
    }
    ...
    bool Has(const K& key) const {
        size_t index = hasher(key) % maps.size();
        lock_guard<mutex> g(maps[index].m);
        return maps[index].data.count(key) > 0;
    }

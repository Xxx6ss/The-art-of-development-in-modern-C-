...
// Создаём шаблон класса Collider, только в нём будет переопределяться метод Collide
//template <typename T>
//struct Collider : GameObject {
//    bool Collide(const GameObject& that) const final {
//        // Статически приводим тип *this к типу const T&, потому что мы знаем, // что T — наш наследник (см. ниже)
